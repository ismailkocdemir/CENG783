...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    relu_out = np.maximum(0, hidden_out)
    #drop = (np.random.rand( *relu_out.shape ) < dropout) / dropout
    #relu_out *= drop
    ...
    reg_term = 0.5 * reg * ( np.sum(W1 * W1) + np.sum(W2 * W2) )
    loss = reg_term - np.sum( np.log(scores[np.arange(N), y]) ) / float(N)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    grads['W2'] = reg * W2 +  np.dot(relu_out.T, scores)/ float(N)
    grads['b2'] = np.sum(scores, axis = 0) / float(N)
    relu_deriv = np.dot( scores, W2.T )
    relu_deriv[ relu_out <= 0 ] = 0 # Derivative of ReLU.
    grads['W1'] = reg * W1 + np.dot( X.T, relu_deriv ) / float(N)
    grads['b1'] = np.sum( relu_deriv, axis = 0 ) / float(N)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
