...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0, dropout = 1.0):
    ...
    #print 'y', y[:5]
    loss = 0.5*np.sum(np.square( difference))+ 0.5 * reg * (np.sum(W1*W1) + np.sum(W2*W2))
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    grads['W2'] = reg * W2 + np.dot( relu_out.T , difference )
    grads['b2'] = np.sum( difference, axis = 0) 
    relu_deriv = np.dot( difference, W2.T ) 
    relu_deriv[ relu_out <= 0 ] = 0 # Derivative of ReLU.
    grads['W1'] = reg * W1 + np.dot( X.T, relu_deriv )
    grads['b1'] = np.sum( relu_deriv, axis = 0 )
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def train(self, X, y, X_val, y_val,
            ...
            reg=1e-5, dropout = 1.0, num_iters=100,
